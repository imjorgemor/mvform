...
export function createFormControl<TFieldValues extends FieldValues = FieldValues>(
  ...
  const _formState: FormState<TFieldValues> = {
    ...
    submitCount: 0,
    // general state
    isValid: !isEmptyObject(_options.errors),
    isDirty: false,
    // rest formState
    isValidating: false,
    touchedFields: {},
  ...
  // state management
  const stateSubject: Subject<Partial<FormState<TFieldValues>>> =
    createSubject<Partial<FormState<TFieldValues>>>();
  ...
  >(
    ...
    if (value === null || value === undefined) {
      fieldValue = '' as KFieldValue;
    } else {
      fieldValue = value;
    }
    _formValues[name] = fieldValue;
  ...
  const getTouchedFields: UseFormGetTouchedFields<TFieldValues> = () => _formState.touchedFields;
